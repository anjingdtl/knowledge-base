...
def run_demo(workdir: Path, keep_workdir: bool = False) -> dict:
    ...
    try:
        ...
        docs_dir = workdir / "docs"
        print(f"✓ 创建了 {len(list(docs_dir.glob('*.md')))} 个测试文档")
...
"""
        ...
        print("\n" + "=" * 60)
        print("演示结果:")
        print(f"  初始搜索命中: {'✓' if results['initial_hit'] else '✗'}")
        print(f"  增量更新成功: {'✓' if results['incremental_update'] else '✗'}")
        print(f"  引用完整性: {'✓' if results['citation_complete'] else '✗'}")
        print("=" * 60)
        
        return results
        
    except Exception as e:
        print(f"\n✗ 演示失败: {e}", file=sys.stderr)
        import traceback
        traceback.print_exc()
        return results
