...
TITLE_BATCH_PROMPT = """你是一个标题生成助手。请分别判断下列文件名是否已能准确概括对应内容。

## 输出格式
只输出一个 JSON 数组，不要使用 Markdown 代码块，不要解释。数组中每个对象必须包含输入中的 id：
[{{"id": "条目ID", "needs_supplement": false}}]
或
[{{"id": "条目ID", "needs_supplement": true, "supplement": "补充说明（不超过20字）"}}]

## 判断规则
1. 文件名已经清晰表达主题时，needs_supplement 为 false。
2. 文件名模糊或与内容核心主题不一致时，needs_supplement 为 true，并给出精炼补充说明。
3. 不要重复文件名已有信息；不得省略、修改或新增 id。
...
def _extract_json_array(text: str) -> list[dict] | None:
    """Extract the first JSON array from an LLM response without trusting prose around it."""
...
def generate_titles_batch(items: list[dict[str, str]]) -> dict[str, str]:
    """Generate titles for up to ten entries in one LLM request.

    Every input ID receives a deterministic filename-based fallback. This keeps the
    existing rename behavior when the provider fails or returns an incomplete result.
    """
    ...
    for item in items:
        ...
        if content.strip():
            payload.append({"id": item_id, "filename": filename, "content": content})

    titles = {item_id: filename[:60] for item_id, filename in filenames.items()}
    if not payload:
        return titles

    try:
        raw = LLMService().chat(
            [{"role": "user", "content": TITLE_BATCH_PROMPT.format(
                items_json=json.dumps(payload, ensure_ascii=False),
            )}],
            silent=True,
            max_tokens_override=800,
        )
        results = _extract_json_array(_strip_think(raw).strip())
        if results is None:
            return titles
        for result in results:
            ...
            if filename:
                titles[item_id] = assemble_title(filename, result)
    except Exception:
        pass
    return titles
