...
from src.services.indexer import get_vector_coverage, repair_missing_block_vectors
...
class VectorCoverageRepairWorker(QThread):
    ...
    def run(self) -> None:
        try:
            result = repair_missing_block_vectors(progress_callback=self.progress.emit)
...
class MaintenanceView(QWidget):
    ...
    def __init__(self) -> None:
        ...
        self._vector_repair_worker: VectorCoverageRepairWorker | None = None
        self._setup_ui()
        self._refresh_vector_coverage()
    ...
    def _setup_ui(self) -> None:
        ...
        top.addStretch()
        self.lbl_vector_coverage = QLabel("")
        self.lbl_vector_coverage.setObjectName("hintLabel")
        top.addWidget(self.lbl_vector_coverage)
        self.btn_repair_vectors = QPushButton("修复向量覆盖率")
        self.btn_repair_vectors.setObjectName("accentBtn")
        set_named_icon(self.btn_repair_vectors, "refresh", "on_accent", 14)
        self.btn_repair_vectors.clicked.connect(self._on_repair_vectors)
        top.addWidget(self.btn_repair_vectors)
    ...
    def _refresh_vector_coverage(self) -> None:
        try:
            coverage = get_vector_coverage()
        except Exception as exc:  # noqa: BLE001
            self.lbl_vector_coverage.setText("向量覆盖率: 读取失败")
            logger.warning("Failed to load vector coverage: %s", exc)
            return
        self.lbl_vector_coverage.setText(
            "向量覆盖率: {coverage:.1%} ({covered_blocks}/{total_blocks})".format(
                **coverage
            )
        )
    ...
    def _on_repair_vectors(self) -> None:
        try:
            coverage = get_vector_coverage()
        except Exception as exc:  # noqa: BLE001
            QMessageBox.warning(self, "读取失败", str(exc))
            return
        missing = int(coverage["missing_blocks"])
        if missing == 0:
            QMessageBox.information(self, "无需修复", "当前所有 Block 均已有向量。")
            return
        if QMessageBox.question(
            self,
            "修复向量覆盖率",
            "将调用当前配置的 Embedding 模型补齐 "
            f"{missing} 个缺失向量。该操作可能产生 API 用量。\n\n确认开始修复？",
        ) != QMessageBox.StandardButton.Yes:
            return

        self.btn_repair_vectors.setEnabled(False)
        self.lbl_vector_coverage.setText(f"向量覆盖率: 修复中... (0/{missing})")
        self._vector_repair_worker = VectorCoverageRepairWorker()
        self._vector_repair_worker.progress.connect(self._on_vector_repair_progress)
        self._vector_repair_worker.finished_ok.connect(self._on_vector_repair_finished)
        self._vector_repair_worker.error.connect(self._on_worker_error)
        self._vector_repair_worker.start()

    def _on_vector_repair_progress(self, current: int, total: int) -> None:
        self.lbl_vector_coverage.setText(f"向量覆盖率: 修复中... ({current}/{total})")

    def _on_vector_repair_finished(self, result: dict) -> None:
        self._vector_repair_worker = None
        self.btn_repair_vectors.setEnabled(True)
        self._refresh_vector_coverage()
        message = (
            f"向量覆盖率: {result['coverage_before']:.1%} → "
            f"{result['coverage_after']:.1%}\n"
            f"已补齐: {result['repaired']}\n"
            f"失败: {result['failed']}"
        )
        if result["errors"]:
            message += f"\n首个错误: {result['errors'][0]['error']}"
        QMessageBox.information(self, "向量修复完成", message)
