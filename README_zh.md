...
**v1.4.0**（当前）— Phase 3 管线加固、第6轮 MCP 稳定性/召回测试（13 项 BUG 修复）、trace token 可观测性。Python 全量测试：`951 passed, 1 skipped`。

版本 `1.3.1` 已于 2026-06-13 完成全仓库健康审查：

- Python 全量测试 `828 passed, 2 skipped`
- `src`、`tests`、`evals`、`tools`、`scripts` 扩展 Ruff 检查全绿
- 157 个源码/工具文件通过 mypy
- React/Vite 生产构建通过
- GitHub Actions 的 Test、Lint、Frontend Build、Retrieval Eval、Docker Build 五项 Linux 门禁全部通过
- 检索门禁通过：Recall@5 `0.8667`、MRR `0.7800`、nDCG@10 `0.7938`、No-Answer Accuracy `0.6667`、引用位置完整性 `1.0000`
- 本地检索端到端 Demo 的首次检索、增量更新和引用校验全部通过
