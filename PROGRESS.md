...
- **0.6→0.7 需真实领域数据 + 填充 `data/lexical_zh_dict.txt`/`synonyms.txt` + `shinehe index --reindex`**（符 W3 handoff §5.2 预判，defer 用户环境）。引擎已就绪，真实数据接入即可量化提升。
...
- retrieval_zh Recall@5 0.6→0.7 的真实数据验证（用户环境 + dict/synonyms + reindex）
